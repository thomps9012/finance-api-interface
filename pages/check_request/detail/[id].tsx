...
import { APPROVE_CHECK_REQUEST, ARCHIVE_CHECK_REQUEST, REJECT_CHECK_REQUEST } from "../../../graphql/mutations"
...
export const getServerSideProps = async (context: GetServerSidePropsContext) => {
  ...
  const grant = await client.query({ query: GET_GRANT, variables: {id: res.data.check_request_detail.grant_id} })
  return {
    props: {
...
export default function RecordDetail({ recorddata, user_permissions, userID, jwt, grant_info }: { grant_info: GrantInfo, jwt: string, recorddata: CheckDetail, user_permissions: string[], userID: string }) {
  console.log(recorddata)
  const { id, current_user, category, receipts, purchases, created_at, credit_card, current_status, date, description, grant_id, is_active, order_total, user_id, action_history } = recorddata;
  ...
  const client = createClient(jwt);
  // need to update these two to match the new input format
  const approveRequest = async (e: any) => {
    e.preventDefault();
    const res = await client.mutate({ mutation: APPROVE_CHECK_REQUEST, variables: { request_type: request_type, request_id: id } })
    res.data.approve_request ? router.push('/me/inbox') : null;
  }
  const rejectRequest = async () => {
    const res = await client.mutate({ mutation: REJECT_CHECK_REQUEST, variables: { request_type: request_type, request_id: id } })
    res.data.reject_request ? router.push('/me/inbox') : null;
  }
  const archiveRequest = async () => {
    const res = await client.mutate({ mutation: ARCHIVE_CHECK_REQUEST, variables: { request_type: request_type, request_id: id } })
    res.data.archive_request ? router.push('/me') : null;
  }
  return <main className={styles.main} id={is_active ? `active` : `inactive`}>
    {user_permissions.find(() =>"ADMIN") != undefined && current_user === userID && current_status != 'REJECTED' && <div className='button-row'>
      <button onClick={approveRequest}>Approve</button>
      <button onClick={rejectRequest}>Reject</button>
    </div>}
    <h2>{grant_info.name}</h2>
    <h1 className={current_status}>{category} Check Request</h1 >
    <div className="hr" />
    ...
    <br />
    {(user_permissions.find(() =>"FINANCE_TEAM") != undefined || userID === user_id) && <button onClick={archiveRequest}>Archive Request</button>}
    <div className="hr" />
